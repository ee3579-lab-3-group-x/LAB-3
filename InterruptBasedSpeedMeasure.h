	...
	virtual double getRPMandUpdate()
	{
		if(enabled)
		{
			...
			if(success)
			{
				...
				if(interval_ms==0)
				{
					//Serial.println(" ========> interval set to zero: force to 1");
					interval_ms=1;
				}
...
public:
	...
	void reset(){RPM_sma_array.reset_array();}

};


// this object (based on the SMA) avoids resetting interrupt counts above certain frequency 
// speed can be read from this object at any frequency without causing numeric instability
class InterruptSpeedMeasure_SMA_maxfreq : public InterruptSpeedMeasure_SMA{
protected:	
	static const unsigned long default_min_inter_measurement_ms=100;
	static const unsigned long absolute_min_inter_measurement_ms=10;
	IntervalCheckTimer minInterMeasurement;
	double prevRPMval; 
public:
	// default consructor (implicitely calls the parent constructor)
	InterruptSpeedMeasure_SMA_maxfreq() {minInterMeasurement.setInterCheck(default_min_inter_measurement_ms);prevRPMval=0;}
	
	// this is to allocate an sma array of chosen size. 
	// If not done, a default size array will be allocated at first use 
	void setupMinInterMeasurement(unsigned long inp_inter_check_ms)
	{
		if(inp_inter_check_ms>absolute_min_inter_measurement_ms)
			minInterMeasurement.setInterCheck(inp_inter_check_ms);
	}
	...
	virtual double getRPMandUpdate()
	{
		if(enabled)
		{
			if(minInterMeasurement.isMinChekTimeElapsedAndUpdate())
			{
				// get the current RPM reading;
				// use the "parent" funciton 
				prevRPMval = InterruptSpeedMeasure_SMA::getRPMandUpdate();
			}
			return prevRPMval;
		}
		else
